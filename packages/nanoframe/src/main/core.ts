...
export class RpcClient {
  ...
  constructor(child: ExecaChildProcess) {
    ...
    if (!child.stdout || !child.stdin)
      throw new Error("Child process pipes not attached");
    this.readLoop(child.stdout);
  }

  private async readLoop(stream: Stream.Readable) {
    let buf = "";
  ...
  kill(signal: number | undefined = undefined) {
    this.child.kill(signal);
  }

  onNotify(cb: (method: string, params: Json) => void) {
    this.notifyCb = cb;
  }
  ...
  static launch(): {
    ...
    const preferLocalCargo =
      isDev || (hasLocalCore && process.env.NANOF_FORCE_PLATFORM !== "1");
    if (preferLocalCargo)
      child = execa(
        "cargo",
        ["run", "--quiet", "--release", "--bin", "nanoframe-core"],
        { stdio: ["pipe", "pipe", "inherit"], cwd: coreDir },
      );
    else if (platformBin)
      child = execa(platformBin, [], { stdio: ["pipe", "pipe", "inherit"] });
    else
      child = execa("nanoframe-core", [], {
...
function resolvePlatformPackageBinary(): string | null {
  const binName =
    process.platform === "win32" ? "nanoframe-core.exe" : "nanoframe-core";
